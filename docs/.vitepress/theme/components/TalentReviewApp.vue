      ...
      >
        ...
        <nav class="flex flex-1 flex-col gap-1">
          ...
          <button
            ...
            @click="setView('cases')"
          >
            <span class="text-base">📚</span> 案例与规则引擎
          </button>
        </nav>
      ...
      <main class="min-w-0 flex-1 overflow-y-auto bg-slate-100 p-6 lg:p-8">
        ...
        <template v-else-if="view.tab === 'cases'">
          <div class="mx-auto max-w-6xl space-y-8">
            ...
            >
              <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-indigo-500">Engine Hub</p>
              <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">盘点引擎中心</h1>
              <p class="mt-2 max-w-2xl text-sm text-slate-500">管理 AI 判定规则与测试数据集</p>
            </header>

            <!-- 规则库：可视化策略中心 -->
            <section class="space-y-4">
              <div class="flex flex-wrap items-center justify-between gap-3">
                <h2 class="text-lg font-bold text-slate-900">盘点规则库</h2>
                <div
                  class="flex flex-wrap items-center gap-2 rounded-full border border-slate-100 bg-white px-3 py-1.5 text-[11px] text-slate-600 shadow-sm"
                >
                  <span class="font-mono font-semibold text-slate-800">{{ ruleEngineMeta.version }}</span>
                  <span class="text-slate-300">|</span>
                  <span
                    class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 font-medium text-emerald-700 ring-1 ring-emerald-100"
                  >
                    <span class="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />
                    状态: {{ ruleEngineMeta.status }}
                  </span>
                  <span class="text-slate-300">|</span>
                  <span>最后同步: {{ ruleEngineMeta.syncedAt }}</span>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                <!-- 数据清洗 -->
                <article
                  class="flex flex-col rounded-2xl border border-slate-100 bg-white p-5 shadow-sm ring-1 ring-slate-50"
                >
                  <div class="mb-4 flex items-start justify-between gap-2">
                    <div>
                      <span
                        class="inline-flex rounded-md bg-sky-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-sky-700 ring-1 ring-sky-100"
                        >数据清洗</span
                      >
                      <h3 class="mt-2 text-base font-bold text-slate-900">输入质量与置信度</h3>
                      <p class="mt-1 text-xs text-slate-500">来自规则引擎的阈值与清洗策略</p>
                    </div>
                    <span class="text-xl">🧹</span>
                  </div>
                  <ul class="space-y-3">
                    <li
                      v-for="row in ruleCleaningRows"
                      :key="row.title"
                      class="rounded-xl border border-slate-100 bg-slate-50/50 p-3"
                    >
                      <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-medium text-slate-700">{{ row.title }}</span>
                        <span
                          class="rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-slate-600 ring-1 ring-slate-200"
                          >{{ row.badge }}</span
                        >
                      </div>
                      <div class="mt-2 flex items-baseline gap-1">
                        <span class="text-lg font-bold tabular-nums text-slate-900">{{ row.value }}</span>
                      ...
                      <div class="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-200/80">
                        <div
                          class="h-full rounded-full bg-gradient-to-r from-sky-400 to-indigo-500 transition-all"
                          :style="{ width: `${row.pct}%` }"
                        />
                      </div>
                      <p class="mt-2 text-[11px] leading-relaxed text-slate-500">{{ row.desc }}</p>
                    </li>
                  </ul>
                </article>
                ...
                <!-- 能力字典 -->
                <article
                  class="flex flex-col rounded-2xl border border-slate-100 bg-white p-5 shadow-sm ring-1 ring-slate-50 lg:col-span-2"
                >
                  <div class="mb-4 flex items-start justify-between gap-2">
                    <div>
                      <span
                        class="inline-flex rounded-md bg-violet-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-violet-700 ring-1 ring-violet-100"
                        >能力字典</span
                      >
                      <h3 class="mt-2 text-base font-bold text-slate-900">分层维度权重</h3>
                      <p class="mt-1 text-xs text-slate-500">基层 / 中层 / 高层 · 能力维度占比</p>
                    </div>
                    <span class="text-xl">📐</span>
                  </div>
                  <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div
                      v-for="tier in competencyTiers"
                      :key="tier.tier"
                      class="rounded-xl border border-slate-100 bg-slate-50/40 p-4"
                    >
                      <div class="mb-3 flex items-center justify-between">
                        <span class="text-xs font-bold text-slate-800">{{ tier.label }}</span>
                        <span
                          class="rounded-full bg-white px-2 py-0.5 text-[10px] font-medium text-slate-500 ring-1 ring-slate-200"
                          >{{ tier.tier }}</span
                        >
                      </div>
                      <div class="space-y-2.5">
                        <div v-for="dim in tier.dimensions" :key="dim.key" class="space-y-1">
                          <div class="flex justify-between text-[10px] text-slate-500">
                            <span>{{ dim.label }}</span>
                            <span class="font-semibold tabular-nums text-slate-700">{{ dim.pct }}%</span>
                          </div>
                          <div class="h-1.5 overflow-hidden rounded-full bg-slate-200/80">
                            <div
                              class="h-full rounded-full bg-violet-500/90"
                              :style="{ width: `${dim.pct}%` }"
                            />
                          </div>
                        </div>
                ...
                <!-- 九宫格判定 -->
                <article
                  class="flex flex-col rounded-2xl border border-slate-100 bg-white p-5 shadow-sm ring-1 ring-slate-50 lg:col-span-3"
                >
                  <div class="mb-4 flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <span
                        class="inline-flex rounded-md bg-amber-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-800 ring-1 ring-amber-100"
                        >九宫格判定标准</span
                      >
                      <h3 class="mt-2 text-base font-bold text-slate-900">绩效 × 潜力分档区间</h3>
                      <p class="mt-1 text-xs text-slate-500">将 0–100 分映射为高 / 中 / 低三档，再组合为九宫格坐标</p>
                    </div>
                    <span class="text-xl">⊞</span>
                  </div>
                  <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <div
                      v-for="band in gridBands"
                      :key="band.key"
                      class="flex flex-col rounded-xl border border-slate-100 bg-gradient-to-br from-white to-slate-50/80 p-4 shadow-sm"
                    >
                      <div class="flex items-center justify-between gap-2">
                        <span class="text-sm font-bold text-slate-900">{{ band.label }}</span>
                        <span
                      ...
                      <p class="mt-2 text-[11px] leading-relaxed text-slate-500">{{ band.hint }}</p>
                      <div class="mt-3 text-[10px] font-medium uppercase tracking-wide text-slate-400">
                        维度 · {{ band.key }}
                      </div>
                    </div>
                  </div>
            ...
            <section class="space-y-4">
              <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 class="text-lg font-bold text-slate-900">模拟案例库</h2>
                  <p class="text-xs text-slate-500">静态资源：<code class="rounded bg-slate-100 px-1 py-0.5">public/data/test_cases.json</code></p>
                </div>
                <button
                  type="button"
                ...
                >
                  ...
                  <span v-else>导入 JSON 案例</span>
                </button>
              </div>
              <p v-if="importError" class="text-sm text-red-600">{{ importError }}</p>
              <ul
                v-if="importedCases.length > 0"
                class="grid list-none grid-cols-1 gap-4 p-0 md:grid-cols-2 lg:grid-cols-3"
              >
                <li
                  v-for="c in importedCases"
                  :key="c.id"
                  class="group flex flex-col rounded-xl border border-slate-100 bg-white p-4 shadow-sm ring-1 ring-slate-50 transition-all hover:-translate-y-1 hover:shadow-md hover:ring-2 hover:ring-indigo-500"
                >
                  <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
                      <h3 class="truncate font-bold text-slate-900 group-hover:text-indigo-700">{{ c.name }}</h3>
                      <p class="text-[11px] text-slate-500">{{ c.department }} · {{ c.level }}</p>
                    </div>
                    <span
                      class="shrink-0 rounded-lg px-2 py-1 text-[10px] font-black uppercase tracking-wide ring-1"
                      :class="expectedBoxBadgeClass(c.expected_box)"
                    >
                      {{ c.expected_box }}
                    </span>
                  </div>
                  <p class="mt-3 line-clamp-3 text-sm leading-relaxed text-slate-600 italic">
                    "{{ c.raw_review }}"
                  </p>
                  <div class="mt-3 flex flex-wrap gap-1">
                    <span
                      v-for="(tag, ti) in c.manual_tags"
                      :key="`${c.id}-tag-${ti}`"
                      class="rounded-full border border-slate-100 bg-slate-50 px-2 py-0.5 text-[10px] text-slate-600"
                      >#{{ tag }}</span
                    >
                  </div>
                  <div
                    v-if="c.pred_performance != null && c.pred_potential != null"
                    class="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4"
                  >
                    <div class="rounded-lg border border-slate-100 bg-slate-50/90 p-2.5">
                      <div class="flex items-center justify-between text-[10px] text-slate-500">
                        <span class="flex items-center gap-1 font-medium">📈 绩效</span>
                        <span class="font-bold tabular-nums text-slate-900">{{ c.pred_performance }}</span>
                      </div>
                      <div class="mt-1.5 h-2 overflow-hidden rounded-full bg-slate-200">
                        <div
                          ...
                          :style="{ width: `${Math.min(100, Math.max(0, c.pred_performance))}%` }"
                        />
                      </div>
                    </div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/90 p-2.5">
                      <div class="flex items-center justify-between text-[10px] text-slate-500">
                        <span class="flex items-center gap-1 font-medium">🌱 潜力</span>
                        <span class="font-bold tabular-nums text-slate-900">{{ c.pred_potential }}</span>
                      </div>
                      <div class="mt-1.5 h-2 overflow-hidden rounded-full bg-slate-200">
                        <div
                          class="h-full rounded-full bg-gradient-to-r from-emerald-400 to-teal-500"
                          :style="{ width: `${Math.min(100, Math.max(0, c.pred_potential))}%` }"
                        />
                      </div>
                    </div>
                  </div>
              ...
              <p v-else class="rounded-xl border border-dashed border-slate-200 bg-white/80 p-8 text-center text-sm text-slate-400 shadow-sm">
                点击「导入 JSON 案例」从站点静态资源加载数据，导入后将同步至仪表盘人才池。
              </p>
            </section>
          </div>
        </template>
...
const ruleEngineMeta = {
  version: 'V2.0',
  status: '运行中',
  syncedAt: '2026-03-24'
} as const

function labelFromWeightKey(key: string): string {
...
const ruleCleaningRows = computed(() => {
  const c = SCORING_RULES.cleaning
  return [
    {
      title: '最少字数',
      badge: '输入',
      value: String(c.minCharCount),
      unit: '字',
      desc: '评语过短将触发低置信度提示，引导 HR 补充素材',
      pct: Math.min(100, Math.round((c.minCharCount / 100) * 100))
    },
    {
      title: '置信度阈值',
      badge: '模型',
      value: String(c.confidenceThreshold),
      unit: '',
      desc: '低于阈值时建议人工复核或二次采集',
      pct: Math.min(100, Math.round(c.confidenceThreshold * 100))
    }
  ]
})
...
const competencyTiers = computed(() => {
  ...
  const toDims = (obj: Record<string, number>) =>
    Object.entries(obj).map(([key, v]) => ({
      key,
      label: labelFromWeightKey(key),
      pct: Math.round(v * 100)
    }))
  return [
    { tier: '基层', label: 'Junior', dimensions: toDims(w.junior) },
    { tier: '中层', label: 'Middle', dimensions: toDims(w.middle) },
    { tier: '高层', label: 'Senior', dimensions: toDims(w.senior) }
  ]
})

const gridBands = computed(() => {
  const m = SCORING_RULES.boxMapping
  return [
    {
      key: 'high',
      label: '高档',
      range: `${m.high.min}–${m.high.max}`,
      hint: '高绩效 / 高潜力主战场，优先投入与激励',
      badgeClass: 'bg-emerald-100 text-emerald-800 ring-emerald-200'
    },
    {
      key: 'mid',
      label: '中档',
      range: `${m.mid.min}–${m.mid.max}`,
      hint: '中坚力量与观察区，适合校准与培养',
      badgeClass: 'bg-amber-100 text-amber-800 ring-amber-200'
    },
    {
      key: 'low',
      label: '低档',
      range: `${m.low.min}–${m.low.max}`,
      hint: '待改进与风险关注，需要干预与辅导',
...
/** 将 test_cases.json 单条转为 TalentRecord，供仪表盘 / 九宫格与 fullData 同源 */
function testCaseJsonToRecord(c: TestCaseJson): TalentRecord {
  const perf =
    c.pred_performance === undefined || c.pred_performance === null
      ? 50
      : Math.round(Math.max(0, Math.min(100, c.pred_performance)))
  const pot =
    c.pred_potential === undefined || c.pred_potential === null
      ? 50
      : Math.round(Math.max(0, Math.min(100, c.pred_potential)))
  const gridPosition =
    c.expected_box && GRID_MAP[c.expected_box] ? c.expected_box : getGridPos(perf, pot)
  ...
  const aiAnalysis: AIAnalysis = {
    ...
    modelJudgment: {
      performanceScore: perf,
      potentialScore: pot,
      confidence: { performance: '中', potential: '中' }
    },
    suggestion: {
      gridPosition,
      talentTags: tags,
